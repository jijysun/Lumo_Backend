...
| # | 무엇을 | 어디서 | 몇 회차 | 참조 절 |
|---|---|---|---:|---|
| **0** | 대조군 이미지 굽기·푸시 (A·B) | 🖥 로컬 | — | [대조군 이미지 굽기](#대조군-이미지-굽기-한-번만) |
| **1** | **파일럿** (C) — 하네스 검증, **데이터 버림** | 🖥 로컬 | 1 | [EC2 측정 순서](#ec2-측정-순서--파일럿-먼저-그다음-시나리오별-블록) |
| **2** | **배출 상한 캘리브레이션** (C) — `RATE` 사다리 | 🖥 로컬 | 3 | [배출 상한 캘리브레이션](#배출-상한-캘리브레이션--본-측정-전에-한-번) |
| **3** | **파라미터 재결정** — 상한을 알았으니 표를 다시 채운다 | 📝 | — | [회차별 권장 파라미터](#회차별-권장-파라미터) |
...
### ⛔ 상한을 넘긴 회차는 반드시 유실이 난다 — `MAX_LEN` 링버퍼
...
```
백로그(t) = (도착률 − 배출률) × t        배출률 실측 461건/초 (20260902)
백로그 > 10,000 이 되는 시각  t = 10,000 ÷ (도착률 − 461)
```

| 회차 | 도착 총량 | 부하 중 배출 | 링버퍼 보관 | **예상 유실** | 10,000 도달 |
|---|---:|---:|---:|---:|---:|
| `RATE=500` · 1m | 30,000 | 27,660 | 9,712 (실측) | **0** ← 가까스로 안 넘김 | — |
| `RATE=1000` · 1m | 60,000 | 27,660 | ~10,000 | **≈ 22,000** | 약 19초 |
| `RATE=2000` · 1m | 120,000 | 27,660 | ~10,000 | **≈ 82,000** | 약 6.5초 |
